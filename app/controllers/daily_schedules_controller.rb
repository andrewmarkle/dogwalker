class DailySchedulesController < ApplicationController
  ...
  def index
    @daily_walks = DailySchedule.all
    @schedule_by_date = @daily_walks.group_by(&:published_on)
  end
  ...
  private
    ...
    def daily_schedule_params
      params.require(:daily_schedule).permit(:date_and_time, :dog_name, :people_name)
    end
end
